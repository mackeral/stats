...
function startHarvest(fromParam){
	allowedFormats.forEach(function(format) {
		harvestURL = baseURL + 'metadataPrefix=' + format + fromParam + "&set=publication:facpubs";
		...
		records[format] = [];
			...
			res.on('end', function(){
				...
				parseString(trimmedRecord, { trim: true }, function(err, result){
					...
					else if(result['OAI-PMH']['ListRecords'] == undefined) {
						console.log(format + ": empty result set");
					} else {
